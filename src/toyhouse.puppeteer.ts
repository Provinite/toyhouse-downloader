...
export async function loginToToyhouse(page: Page) {
  ...
  if (!loggedIn) {
    ...
    if (page.url().endsWith("/~account/login")) {
      await screenshot(page, { path: "./login-failed.jpg" });
      throw new Error(
        "Error logging in. Please check your username and password. See login-failed.jpg for a screenshot of what happened."
      );
    }
  } else {
